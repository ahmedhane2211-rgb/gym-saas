...
const PlanModal = ({
  ...
  const availableFeatures = Array.isArray(featuresData)
    ? featuresData
    : featuresData?.data || [];

  const { data: freezePlansResponse } = useGetFreezePlansQuery();
  const freezePlans = Array.isArray(freezePlansResponse)
    ? freezePlansResponse
    : freezePlansResponse?.data || [];
  ...
  useEffect(() => {
    if (initialData) {
      reset({
        ...
        freeze_plan_id:
          initialData.freeze_plan_id || initialData.freezePlanId || "",
        features:
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            >
              <option value="" className="bg-white dark:bg-dark">
                {t("select_option") || "اختر..."}
              </option>
              {freezePlans.map((plan) => (
                ...
                >
                  {plan.name} ({plan.days} {t("days")} - {t("max_uses")}:{" "}
                  {plan.max_uses})
                </option>
              ))}
            </select>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className="flex flex-col md:flex-row items-end gap-3 p-4 bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 rounded-2xl animate-in slide-in-from-top-2 duration-300"
                >
                  <div className="flex-1 w-full space-y-2">
                    ...
                    <select
                      {...register(`features.${index}.featuresId`, {
                        required: true,
                      })}
                    ...
                    >
                      <option value="">
                        {t("select_feature") || "Select Feature"}
                      </option>
              ...
              {fields.length === 0 && (
                <p className="text-center py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest  border-2 border-dashed border-gray-100 dark:border-white/5 rounded-2xl">
