        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
              {t("plan_details") || "Subscription Plan Details"}
            </p>
          </div>
          <span
            className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${plan.is_active ? "bg-orange/10 border-orange/20 text-orange" : "bg-gray-500/10 border-gray-500/20 text-gray-500"}`}
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 space-y-1">
            ...
            <p className="text-xl font-black text-gray-900 dark:text-white ">
              {plan.price}{" "}
              <span className="text-[10px] font-bold text-gray-400">EGP</span>
            </p>
          </div>
          <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 space-y-1">
        ...
        {plan.freeze_plan_id && (
          <div className="p-4 rounded-2xl bg-orange/5 border border-orange/10 space-y-2">
            ...
            <p className="text-[10px] font-bold text-gray-500">
              {plan.freeze_days} {t("days")} - {t("max_uses")}:{" "}
              {plan.freeze_max_uses}
            </p>
          </div>
        )}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
              {t("included_features") || "Included In This Plan"}
            </h4>
          </div>
          <div className="space-y-3">
            {plan.features?.map((f, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-4 rounded-2xl bg-blue/5 border border-blue/10"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-blue/20 flex items-center justify-center text-blue">
                    <CheckCircle2 size={16} />
                  </div>
                  <span className="text-xs font-black text-gray-900 dark:text-gray-200 uppercase tracking-tight">
                    {f.name}
                  </span>
                </div>
                <span className="px-3 py-1 bg-white dark:bg-black/40 rounded-lg text-[10px] font-black text-blue shadow-sm border border-blue/5">
                  {f.value}
                </span>
              </div>
            ))}
