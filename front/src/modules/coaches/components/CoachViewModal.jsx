      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[#121212] border border-gray-200 dark:border-white/5 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="relative pt-12 px-8 pb-8 space-y-8">
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div>
                <h3 className="text-2xl font-black text-gray-900 dark:text-white italic uppercase tracking-widest">{coach.user?.full_name}</h3>
          ...
          <div className="grid grid-cols-1 gap-6 pt-4">
            <div className="grid grid-cols-2 gap-4">
                <DetailItem icon={<Award size={16}/>} label={t('specialty')} value={coach.speciality || coach.specialty || 'N/A'} />
                <DetailItem icon={<Banknote size={16}/>} label={t('salary')} value={`${coach.salary} EGP` || 'N/A'} />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <DetailItem icon={<Shield size={16}/>} label={t('gender')} value={t(coach.user?.gender) || 'N/A'} />
                <DetailItem 
                    icon={<Activity size={16}/>} 
                    label={t('status')} 
                    value={
                        <span className={coach.user?.is_active ? 'text-blue' : 'text-gray-500'}>
            ...
            <div className="grid grid-cols-2 gap-4">
                <DetailItem icon={<Mail size={16}/>} label={t('email')} value={coach.user?.email || 'N/A'} />
                <DetailItem icon={<Phone size={16}/>} label={t('phone')} value={coach.user?.phone || 'N/A'} />
            </div>
