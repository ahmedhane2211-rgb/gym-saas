...
import { BriefcaseBusiness, Plus, Banknote, Eye, CheckCircle2 } from "lucide-react";
...
import { usePaySalaryMutation, useGetAllPaymentsQuery } from "../services/SalaryPaymentSlice";
...
const Salaries = () => {
    ...
    // User creation date
    const createdDate = userProfile?.created_at ? new Date(userProfile.created_at) : new Date();
    ...
    const { data: bonusResponse, isLoading: bonusLoading } = useGetMonthlyBonusDeductionsQuery({ month: selectedMonth, year: selectedYear });
    const { data: withdrawalsResponse, isLoading: withdrawalsLoading } = useGetEmployeeWithdrawalsQuery();
    const { data: paymentsResponse, isLoading: paymentsLoading } = useGetAllPaymentsQuery();

    const [addBonusDeduction, { isLoading: isAddingBonus }] = useAddBonusDeductionMutation();
    ...
    const filteredEmployees = useFilter(employees, searchTerm, ["name", "job_number"]);
    ...
    const handlePaySalary = async (employee) => {
        if (window.confirm(t("confirm_pay_salary", { name: employee.name }) || `هل تريد دفع راتب ${employee.name} للشهر الحالي؟`)) {
            try {
                await paySalary({
    ...
    const calculateNetSalary = (employee) => {
        ...
        const totalBonuses = empBonuses.filter(b => b.type === "bonus").reduce((acc, b) => acc + Number(b.value), 0);
        const totalDeductions = empBonuses.filter(b => b.type === "deduction").reduce((acc, b) => acc + Number(b.value), 0);
        const totalWithdrawals = empWithdrawals.reduce((acc, w) => acc + Number(w.value), 0);

        return Number(employee.basic_salary) + totalBonuses - totalDeductions - totalWithdrawals;
    };

    const isPaidThisMonth = (employeeId) => {
        return payments.some(p => p.employee_id === employeeId && Number(p.month) === selectedMonth && Number(p.year) === selectedYear);
    };
    ...
    const columns = [
        {
            ...
            render: (employee) => (
                <div>
                    <p className="text-gray-900 dark:text-white font-black text-sm uppercase tracking-tight italic">{employee.name}</p>
        ...
        },
        {
            header: "basic_salary",
            render: (employee) => (
                <span className="text-gray-900 dark:text-white text-xs font-bold">{Number(employee.basic_salary).toLocaleString()}</span>
            )
        },
        {
            header: "net_salary",
        ...
        },
        {
            ...
            render: (employee) => {
                ...
                return (
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${paid ? "bg-green-500/10 text-green-500" : "bg-yellow-500/10 text-yellow-500"}`}>
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div className="space-y-2">
                    <h1 className="text-5xl font-black text-gray-900 dark:text-white tracking-widest italic uppercase">{t("salaries")}</h1>
                    <p className="text-gray-600 dark:text-gray-500 text-xs font-bold uppercase tracking-widest max-w-md leading-relaxed">
                        {t("manage_salaries_desc")}
                    </p>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        >
                            {years.map((y) => (
                                <option key={y} value={y} className="dark:bg-dark text-gray-900 dark:text-white">
                                    {y}
                                </option>
                            ))}
                        </select>
                        ...
                        >
                            {months.map((m) => (
                                <option key={m} value={m} className="dark:bg-dark text-gray-900 dark:text-white">
                                    {m < 10 ? `0${m}` : m}
                                </option>
                            ))}
                        </select>
            ...
            <DataTable
                ...
                isLoading={empLoading || bonusLoading || withdrawalsLoading || paymentsLoading}
                actions={false}
                title={t("salaries")}
            />
{/* 
