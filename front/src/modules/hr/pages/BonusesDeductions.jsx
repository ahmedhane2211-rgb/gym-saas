...
const BonusesDeductions = () => {
  ...
  const records = Array.isArray(bonusResponse)
    ? bonusResponse
    : bonusResponse?.data || [];
  const filteredRecords = useFilter(records, searchTerm, [
    "employee_name",
    "notes",
  ]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${row.type === "bonus" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
        >
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <SectionTitle title="bonuses_deductions" description="manage_bonuses_deductions_desc" t={t} />
      </div>
