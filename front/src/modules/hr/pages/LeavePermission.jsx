...
const LeavePermission = () => {
  ...
  const filteredRequests = useFilter(filteredByStatus, searchTerm, [
    "employee_name",
    "leave_name",
  ]);
  ...
  const handleReject = async (id) => {
    if (
      window.confirm(
        t("confirm_reject") || "Are you sure you want to reject this request?",
      )
    ) {
      try {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (req) => (
        ...
        >
          <div
            className={`w-1 h-1 rounded-full ${req.status === "pending" ? "bg-orange animate-pulse" : req.status === "approved" ? "bg-blue animate-pulse" : "bg-red-500"}`}
          />
          {t(req.status)}
        </span>
      ),
  ...
  const exportColumns = [
    ...
    { header: "leave_name", key: "leave_name" },
    {
      header: "from_date",
      key: "from_date",
      render: (req) => formattedDate(req.from_date),
    },
    {
      header: "to_date",
      key: "to_date",
      render: (req) => formattedDate(req.to_date),
    },
  ...
  const stats = [
    {
      ...
      icon: <FileText className="text-orange" />,
      color: "orange",
    },
    {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <SectionTitle title="leaves_permissions" description="manage_leaves_permissions_desc" t={t} />

        <div className="flex gap-4 flex-wrap">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="glass-card p-6 min-w-[150px] flex flex-col justify-between h-32 relative overflow-hidden group"
            >
              <StatsCard stat={stat} />
            </div>
          ))}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <SearchFilter
          ...
          placeholder={t("search_leaves_permissions")}
        />
      ...
      <LeavePermissionModal
        ...
        title={
          editingRequest ? "update_leave_permission" : "add_leave_permission"
        }
      />
