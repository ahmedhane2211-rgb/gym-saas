...
const Admin = () => {
    ...
    const admins = Array.isArray(response) ? response : (response?.data || response?.admins || []);
    ...
    const columns = [
        {
            ...
            render: (admin) => (
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-gray-900 dark:text-white font-black text-sm uppercase tracking-tight italic">{admin.name}</p>
                        <p className="text-gray-500 dark:text-gray-600 text-[9px] font-bold uppercase tracking-widest">ID: ADM-{admin.id}</p>
                    </div>
                </div>
            )
        },
        ...
        },
        {
            header: 'status',
            render: (admin) => (
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] uppercase tracking-widest font-black border ${admin.isActive
                    ? 'bg-orange/10 border-orange/20 text-orange'
                    : 'bg-gray-500/10 border-gray-500/20 text-gray-500'
                    }`}>
                    <div className={`w-1 h-1 rounded-full ${admin.isActive ? 'bg-orange animate-pulse' : 'bg-gray-500'}`} />
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div className="space-y-2">
                    <h1 className="text-5xl font-black text-gray-900 dark:text-white tracking-widest italic uppercase">Admins</h1>
                ...
                <div className="glass-card p-6 min-w-[240px] flex flex-col justify-between h-32 relative overflow-hidden group">
                    ...
                    <p className="text-gray-600 dark:text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">Total Admins</p>
