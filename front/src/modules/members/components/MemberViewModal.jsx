...
const MemberViewModal = ({ isOpen, onClose, member }) => {
  ...
  const [useFeature, { isLoading: isUsingFeature }] =
  ...
  const handleUseFeature = async (featureId) => {
    try {
      ...
      toast.success(
        t("feature_used_success") || "Feature usage recorded!"
      );
    } catch (err) {
      toast.error(
        err.data?.message || "Failed to record usage"
      );
    }
  };
  ...
  return (
    ...
    >
      {/* Header Gradient */}
      <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-r from-blue to-blue/50 opacity-10 pointer-events-none" />

      {/* Content */}
      <div className="relative pt-10 px-6 md:px-8 pb-8 space-y-7 max-h-[82vh] overflow-y-auto no-scrollbar">

        {/* ================= PROFILE HEADER ================= */}
        <div className="flex flex-col items-center text-center space-y-4">
          <div className="w-24 h-24 rounded-3xl border-4 border-white dark:border-gray-dark shadow-xl overflow-hidden bg-gray-100 dark:bg-gray-dark -mt-2">
            <img
        ...
        {/* ================= QR CODE ================= */}
        <div className="flex flex-col items-center justify-center p-7 bg-white dark:bg-white/5 rounded-[32px] border border-dashed border-gray-200 dark:border-white/10 space-y-4">
                ...
                {member?.subscription?.freeze_plan_id && (
                  <div>
                    ...
                    <p className="text-xs font-black text-orange uppercase tracking-tight">
                      {Math.max(
                        0,
                        (member?.subscription?.freeze_max_uses ||
                          0) -
                          (member?.subscription?.pauses_count ||
                            0)
                      )}{" "}
                      /{" "}
                      {member?.subscription?.freeze_max_uses ||
              ...
              <div className="space-y-2">
                {member?.subscription?.features?.map((feat) => {
                  ...
                  return (
                    <div
                      key={feat.id}
                      className="flex items-center justify-between gap-3 p-3 bg-white dark:bg-white/5 rounded-2xl border border-blue/5"
                    >
                      <div className="flex flex-col flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex-1 max-w-24 h-1 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                            <div
                      ...
                      <button
                        ...
                        className="shrink-0 px-4 py-2 bg-blue/10 hover:bg-blue text-blue hover:text-black font-black text-[9px] uppercase tracking-widest rounded-xl transition-all disabled:opacity-30 disabled:pointer-events-none"
                      >
                        {feat.used >= feat.total
                          ? t("limit_reached") ||
                            "Limit Reached"
          ...
          <div className="p-6 bg-orange/5 border border-orange/10 rounded-3xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              <DetailItem
                ...
                value={formattedDate(
                  member?.subscription_pause?.from_date
                )}
              />
              ...
              <DetailItem
                ...
                value={formattedDate(
                  member?.subscription_pause?.to_date
                )}
              />
            </div>
            ...
            <div className="flex justify-between items-center pt-2 text-[10px] font-bold text-gray-500">
              <span>
                ...
                {member?.subscription_pause?.days}{" "}
                {t("days")}
              </span>
              ...
              value={
                <span
                  className={
                    ...
                    member?.subscription_pause?.status !==
                      "active"
                      ? "text-blue"
                      : member?.user?.is_active &&
                          member?.subscription_pause?.status ===
