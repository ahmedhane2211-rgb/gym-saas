...
const [filters, setFilters] = useState({
  ...
  const columns = [
      ...
      render: (row) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-lg ${isIncome ? "bg-green-100 text-green-600 dark:bg-green-500/10 dark:text-green-400" : "bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-400"}`}
            >
              {isIncome ? (
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`font-black  ${row.value >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
        >
    ...
    },
    {
      ...
      render: (row) => (
        <div className="font-black text-blue ">
      ...
      render: (row) => (
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="text-xs font-bold">
            {formattedDate(row.created_at || row.date)}
          </span>
        </div>
      ),
    },
  ...
  const totalIncome = reportData.reduce(
    (acc, row) => (row.amount > 0 ? acc + Number(row.amount) : acc),
    0,
  );
  const totalExpense = reportData.reduce(
    (acc, row) => (row.amount < 0 ? acc + Math.abs(Number(row.amount)) : acc),
    0,
  );

  const stats = [
    {
      label: t("current_balance") || "Current Balance",
      value: formatNum(currentBalance) + " EGP",
      icon: <Wallet className="text-blue" />,
      color: "blue",
    },
  ];
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <SectionTitle title="cash_report" description="cash_report_desc" t={t} />

        <div className="flex flex-wrap gap-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="glass-card p-6 min-w-[180px] flex flex-col justify-between h-32 relative overflow-hidden group"
            >
              <StatsCard stat={stat} />
            </div>
          ))}
