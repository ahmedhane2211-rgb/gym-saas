...
import { useGetPumpingMoneyQuery, useAddPumpingMoneyMutation, useDeletePumpingMoneyMutation } from '../services/PumpingMoneySlice';

const PumpingMoney = () => {
    const { t } = useContext(LanguageContext);
    const { data: response, isLoading } = useGetPumpingMoneyQuery();
    const records = Array.isArray(response) ? response : (response?.data || []);
    const [addPumpingMoney, { isLoading: isAdding }] = useAddPumpingMoneyMutation();
    const [deletePumpingMoney] = useDeletePumpingMoneyMutation();
    const [isModalOpen, setIsModalOpen] = useState(false);

    const handleSubmit = async (data) => {
        try {
            await addPumpingMoney(data).unwrap();
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('confirm_delete'))) {
            try {
                await deletePumpingMoney(id).unwrap();
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('manage_pumping_money')}</p>
                </div>
                <Button
            ...
            <MoneyModal
                ...
                isLoading={isAdding}
                title="add_pumping_money"
            />
        </div>
    );
};

export default PumpingMoney;
