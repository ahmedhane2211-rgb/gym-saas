...
const Select = ({
  ...
  return (
    <div
      className={`space-y-2 relative ${isOpen ? "z-[120]" : "z-10"}`}
      ref={dropdownRef}
    >
      {label && (
      ...
      >
        <span
          className={`text-xs font-bold ${selectedOption ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-gray-light/40"}`}
        >
          {selectedOption
            ? selectedOption.label
            : t(placeholder) || t("select_option") || "Select..."}
        </span>
        <ChevronDown
          size={16}
