...
const handlePrint = () => {
 ...
 console.log('settingsData:', settingsData); // ← أضف ده
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; gap:16px; margin-bottom:24px; padding-bottom:16px; border-bottom:2px solid #eee;">
      ${settingsData?.show_logo_in_header && settingsData?.logo 
        ...
        ${settingsData?.show_name_in_header && settingsData?.company_name 
          ? `<h1 style="margin:0; font-size:20px; font-weight:900; text-transform:uppercase; letter-spacing:2px;">${settingsData.company_name}</h1>` 
          : ''}
  ...
  const footerItems = [
    settingsData?.show_phone_in_footer && settingsData?.company_phone && { label: t('phone'), value: settingsData.company_phone },
    settingsData?.show_email_in_footer && settingsData?.company_email && { label: t('email'), value: settingsData.company_email },
    settingsData?.show_whatsapp_in_footer && settingsData?.whatsapp && { label: t('whatsapp'), value: settingsData.whatsapp },
    settingsData?.show_website_in_footer && settingsData?.website && { label: t('website'), value: settingsData.website },
    settingsData?.show_tax_in_footer && settingsData?.tax_number && { label: t('tax_no'), value: settingsData.tax_number },
    settingsData?.show_bank_account_in_footer && settingsData?.bank_account && { label: t('bank_account'), value: settingsData.bank_account },
  ].filter(Boolean);
  ...
  const footerHTML = footerRows.length > 0 ? `
    <div style="margin-top:24px; padding-top:16px; border-top:2px solid #eee;">
      ${footerRows.map(row => `
        <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:12px; margin-bottom:8px;">
          ${row.map(item => `
            <div dir=${isRtl ? 'rtl' : 'ltr'}>
              <p style="font-size:10px; color:#000; text-transform:uppercase; letter-spacing:1px;">${item.label}: ${item.value}</p>
            </div>
          `).join('')}
        </div>
      `).join('')}
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        ${headerHTML}
        <h2 style="text-transform:uppercase; letter-spacing:2px; font-style:italic; margin-bottom:16px;">${title}</h2>
        ${printContent}
        ${footerHTML}
      </body>
    </html>
            ...
            <thead>
              <tr className="border-b border-gray-200 dark:border-white/5 bg-gray-100/50 dark:bg-white/[0.02]">
                <th style={{ fontSize: 'var(--font-size-sm)' }} 
                  className={`px-8 py-6 font-black text-gray-500 dark:text-gray-light/60 uppercase tracking-[0.2em]`}
                >
                  {t('serial')}
                </th>
                {columns.map((col, idx) => (
