...
const Attendance = () => {
  ...
  const attendanceRecords = Array.isArray(response)
    ? response
    ...
    const exportColumns = [
      {
        ...
        key: "member_name",
        render: (record) =>
          record.member?.user?.full_name || record.user?.full_name || "N/A",
      },
      {
        header: "member_id",
        key: "member_id",
  ...
  const stats = [
    {
      label: t("total_check_ins") || "Total Check-ins",
      value: attendanceRecords?.length || 0,
      icon: <ClipboardCheck className="text-orange" />,
      color: "orange",
    },
    {
      label: t("active_now") || "Active Now",
      value: attendanceRecords?.filter((a) => !a.check_out).length || 0,
      icon: <User className="text-blue" />,
      color: "blue",
    },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <SectionTitle title="attendance" description="attendance_desc" t={t} />
