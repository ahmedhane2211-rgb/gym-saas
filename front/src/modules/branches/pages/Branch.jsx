...
const Branch = () => {
    ...
    const columns = [
        ...
        },
        {
            header: 'status',
            render: (branch) => (
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] uppercase tracking-widest font-black border ${branch.is_active
                    ? 'bg-blue/10 border-blue/20 text-blue'
                    : 'bg-gray-500/10 border-gray-500/20 text-gray-500'
                    }`}>
                    <div className={`w-1 h-1 rounded-full ${branch.is_active ? 'bg-blue animate-pulse' : 'bg-gray-500'}`} />
    ...
    const stats = [
        { label: t('total_branches') || 'Total Branches', value: branches?.length || 0, icon: <Building2 className="text-orange" />, color: 'orange' },
        { label: t('active_branches') || 'Active Branches', value: branches?.filter(b => b.is_active).length || 0, icon: <Activity className="text-blue" />, color: 'blue' },
    ];
