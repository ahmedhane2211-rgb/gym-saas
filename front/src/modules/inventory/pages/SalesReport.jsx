...
const SalesReport = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (inv) => {
        ...
        return (
          <span
            className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${statusStyles[inv.status] || "bg-gray-100 text-gray-500"}`}
          >
            {t(inv.status)}
          </span>
        );
  ...
  const totalCompletedInvoices = invoices.filter(
    (inv) => inv.status !== "cancelled",
  );
  const totalSales = totalCompletedInvoices.reduce(
    (acc, inv) =>
      acc + (Number(inv.final_total) - Number(inv.refund_total || 0)),
    0,
  );
  const stats = [
    {
      label: t("total_sales") || "Total Sales",
      value: formatNum(totalSales) + " EGP",
      icon: <TrendingUp className="text-green-500" />,
      color: "green",
    },
    {
      label: t("total_invoices") || "Total Invoices",
