...
const Subscriptions = () => {
    ...
    try {
      if (editingSubscription) {
        if (editingSubscription.id) {
            await updateSubscription({ id: editingSubscription.id, ...data }).unwrap();
  ...
  const handleDelete = async (id) => {
    if (window.confirm(t('confirm_delete') || 'Are you sure you want to delete this gym?')) {
      try {
  ...
  const columns = [
    {
      ...
      render: (sub) => (
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-gray-900 dark:text-white font-black text-sm uppercase tracking-tight italic">{sub.gym_name}</p>
          </div>
        </div>
      )
    },
    {
      header: 'owner_name',
      render: (sub) => (
        <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{sub.user?.full_name || t('no_owner')}</p>
      )
    },
    {
      header: 'email',
      render: (sub) => (
        <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{sub.user?.email || '-'}</p>
      )
    },
    {
      header: 'status',
      render: (sub) => (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] uppercase tracking-widest font-black border ${sub.status === 'active'
          ? 'bg-green-500/10 border-green-500/20 text-green-500'
          : 'bg-red-500/10 border-red-500/20 text-red-500'
          }`}>
          <div className={`w-1 h-1 rounded-full ${sub.status === 'active' ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
  ...
  const stats = [
    { label: t('total_invoices') || 'Total Gyms', value: subscriptions.length, icon: <Building2 className="text-orange" />, color: 'orange' },
    { label: t('active_subscriptions') || 'Active Subscriptions', value: subscriptions.filter(s => s.status === 'active').length, icon: <ShieldCheck className="text-blue" />, color: 'blue' },
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-2">
          <h1 className="text-5xl font-black text-gray-900 dark:text-white tracking-widest italic uppercase">{t('subscriptions_management')}</h1>
