...
const OwnerSidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <aside
      className={`${isCollapsed ? "w-20" : "w-72"} h-screen bg-white dark:bg-dark border-r border-gray-200 dark:border-white/5 flex flex-col fixed ltr:left-0 rtl:right-0 top-0 z-50 overflow-hidden shadow-2xl transition-all duration-300`}
    >
      {/* Logo & Toggle */}
      <div
      ...
      <div className="p-6 space-y-6">
        <div
          className={`pt-4 border-t border-gray-200 dark:border-white/5 ${isCollapsed ? "px-0 flex justify-center" : ""}`}
        >
          <button
            onClick={handleLogout}
            className="flex items-center gap-3 cursor-pointer text-red-600 hover:text-red-900 dark:hover:text-red-500 text-[12px] font-bold uppercase tracking-widest transition-colors py-2"
          >
