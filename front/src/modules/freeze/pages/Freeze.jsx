...
const Freeze = () => {
  ...
  const [updateFreezePlan, { isLoading: isUpdating }] =
    useUpdateFreezePlanMutation();
  ...
  const filteredFreezePlans = useFilter(freezePlans, searchTerm, [
    "name",
    "days",
    "max_uses",
  ]);
  ...
  const handleSubmitFreezePlan = async (data) => {
    try {
      if (editingFreezePlan) {
        ...
        toast.success(
          t("update_success") || "Freeze plan updated successfully",
        );
      } else {
  ...
  const handleDeleteFreezePlan = async (id) => {
    if (
      window.confirm(
        t("confirm_delete") ||
          "Are you sure you want to delete this freeze plan?",
      )
    ) {
      try {
  ...
  const stats = [
    {
      label: t("total_freezes") || "Total Freeze Plans",
      value: freezePlans?.length || 0,
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-gray-600 dark:text-gray-500 text-xs font-bold uppercase tracking-widest max-w-md leading-relaxed">
            {t("manage_freezes_desc") ||
              "Manage subscription freeze packages, days and maximum uses."}
          </p>
        </div>
