...
const Dashboard = () => {
    ...
    const revenueData = useMemo(() => {
        const labels = dashboardData?.monthlyRevenue?.map(item => item.month) || [];
        const data = dashboardData?.monthlyRevenue?.map(item => Number(item.income)) || [];
    ...
    const attendanceData = useMemo(() => {
        const labels = dashboardData?.weeklyAttendance?.map(item => {
            const date = new Date(item.day);
            return date.toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US', { weekday: 'short' });
        }) || [];
        const data = dashboardData?.weeklyAttendance?.map(item => Number(item.count)) || [];
    ...
    }, [dashboardData, language, t]);

    // Process Membership Distribution
    const membershipData = useMemo(() => {
        const labels = dashboardData?.plansDistribution?.map(item => item.plan_name) || [];
        const data = dashboardData?.plansDistribution?.map(item => Number(item.subscriber_count)) || [];
        ...
        <div className="space-y-8 animate-in fade-in duration-700 font-main p-1 lg:p-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter">
                        {t('dashboard')} <span className="text-orange">{t('analytics')}</span>
                    </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 glass-card p-6 min-h-[400px] flex flex-col">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase italic">{t('revenue_growth')}</h3>
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('yearly_overview')}</p>
                        </div>
                ...
                <div className="glass-card p-6 flex flex-col">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase italic">{t('membership_plans')}</h3>
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('active_distribution')}</p>
                        </div>
                    ...
                    <div className="flex-1 relative">
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pt-8">
                            ...
                            <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">{t('total')}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 glass-card p-6 min-h-[350px] flex flex-col">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase italic">{t('attendance_velocity')}</h3>
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('weekly_stats')}</p>
                        </div>
                        <div className="p-2 bg-orange/10 rounded-lg text-orange">
                ...
                <div className="lg:col-span-2 glass-card p-6">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase italic">{t('live_terminal')}</h3>
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('recent_users')}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="flex h-2 w-2 rounded-full bg-green-500 animate-pulse" />
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('live_now')}</span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {dashboardData?.recentLogins?.length > 0 ? (
                            dashboardData.recentLogins.map((user, idx) => (
                                <div key={user.id || idx} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 rounded-2xl group hover:border-orange/30 transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl overflow-hidden grayscale group-hover:grayscale-0 transition-all border border-gray-200 dark:border-white/10 shadow-sm">
                                            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.full_name}`} alt="" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-black text-gray-900 dark:text-white uppercase italic">{user.full_name}</p>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{user.email}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <span className={`px-3 py-1 rounded-lg text-[9px] font-black text-white uppercase tracking-widest ${
...
const StatCard = ({ title, value, trend, isUp, icon, bgColor }) => (
    <div className="glass-card p-6 group hover:scale-[1.02] transition-all duration-300 relative overflow-hidden">
        {/* Background Glow */}
        <div className={`absolute -right-4 -top-4 w-24 h-24 rounded-full opacity-[0.03] group-hover:opacity-[0.08] transition-all ${bgColor}`} />
        
        <div className="flex justify-between items-start mb-4 relative z-10">
            <div className={`p-3 rounded-2xl ${bgColor} shadow-sm group-hover:rotate-12 transition-transform`}>
                {React.cloneElement(icon, { size: 24 })}
            </div>
            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${isUp ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
        ...
        <div className="space-y-1 relative z-10">
            ...
            <h4 className="text-3xl font-black text-gray-900 dark:text-white italic tracking-tighter">{value}</h4>
        </div>
    </div>
);
