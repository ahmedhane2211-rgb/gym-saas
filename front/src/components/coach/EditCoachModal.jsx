...
const EditCoachModal = ({ isOpen, onClose, coach, t }) => {
  const {members} = useSelector((state) => state.members);
  const user_id = members.find(m => m.id === coach?.userId)?.fullName || "";
  const {register,reset,formState:{errors},handleSubmit} = useForm({
    defaultValues: {
      ...
      bio: coach?.bio || "",
      commissionRate: coach?.commissionRate || 10,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="card max-h-[90vh] w-full max-w-lg overflow-y-auto">
        <div className="mb-6 flex items-center justify-between border-b border-slate-200 pb-4 dark:border-slate-700">
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">{t("coaches.editTitle")}</h2>
          <button
            onClick={onClose}
